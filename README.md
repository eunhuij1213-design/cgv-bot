...
감시 날짜는 오늘부터 `LOOKAHEAD_DAYS`(기본 30일) 이내의 **금·토·일**입니다.
...
- **씨네드쉐프 용산 포함** — API 응답에 `siteNo=P013`(씨네드쉐프 용산) 회차가 같이 들어옵니다. `scnsNo`/`scnSseq`가 본관과 겹치기 때문에 회차 키에 `siteNo`를 포함합니다. 제외하려면 `cgv.parse()`에서 필터링하세요.
